...
(function () {
  $.get(CONTEXT_PATH + '/newsignup/findAllSignupByTogetherStamp', {
    ...
    $('.js-btn-step[data-step="next"]').trigger('click');
  });
})();
...
function courseTrHTML(key, item, type) {
  ...
  // 课程模式
  function createCoursePattern(item, list, type) {
    var tmp = '';
    for (var i = 0; i < list.length; i++) {
      var item_ = list[i];
      if (type !== 'create' && item.coursePattern == item_.hiddenValue) {
        tmp +=
  ...
  if (type === 'create') {
    htmlStr =
      ...
      item.companyName +
      '</td>' +
      '<td><input type="text" class="cus-inp js-money" value="0" readonly /></td>' +
      '<td><input type="text" class="cus-inp js-money js-dy-outstanding" value="' +
      c_price +
      '" readonly /></td>' +
      '<td>' +
    ...
    htmlStr =
      ...
      '" required></td>' +
      '<td>' +
      createCoursePattern(item, classClassifyList, type) +
      '</td>' +
      '<td>' +
      '<select disabled name="week_' +
      item.courseStageCode +
      '" class="cus-sel js-sel-week">' +
      weekSelect(item) +
      '</select>' +
      ...
      '" required="true" value="' +
      item.classCode +
      '"/><button disabled type="button" class="btn btn-xs btn-primary js-btn-class btn-success" data-toggle="tooltip" data-placement="bottom" title="' +
      (item.classCode + '(' + item.className + ')') +
      '">已选</button></td>' +
      '<td>' +
      '<button type="button" class="btn btn-xs btn-primary js-btn-discount">优惠</button>&nbsp;' +
      ...
      '" readonly></td>' +
      '<td>' +
      '<button disabled type="button" class="btn btn-xs btn-primary js-btn-books ' +
      ...
      '</button>&nbsp;' +
      '<input disabled type="text" name="cb_' +
      item.courseStageCode +
      '" ck-course-price="' +
      (item.teachingMaterialCost || 0) +
      '" class="input-sm cus-inp btn-inp js-money js-dy-gooks" value="' +
    ...
    return $(htmlStr)
      .data('sdata', item)
      .data('paramsClass', item.classList)
      .data('paramsDiscount', item.discountAmountList)
      .data('paramsGooks', item.goodsList);
  }
}
// 构建 报名课程 HTML
function createCourseHTML(sArr, callback, type) {
  ...
  function courseTable(id) {
    return (
      ...
      '<th style="width: 70px;">课时单价</th>' +
      '<th style="width: 70px;">模式</th>' +
      '<th style="width: 90px;">周中/周末</th>' +
  ...
  $.each(tmp_obj.d, function (key, item) {
    ...
    var _lianbaoHTML = lianbaoHTML(item.lb_data);
...
function js_classPrice($obj) {
  var class_hour = $obj.find('.js-dy-classHour').val() * 1;
  var class_price = $obj.find('.js-dy-price').val() * 1;
...
function js_discountPrice($obj) {
  console.log($obj.find('.js-dy-discount').val());
  var class_total_price = $obj.find('.js-price-course').val() * 1;
  var discount_price = $obj.find('.js-dy-discount').val() * 1;

  var res = COM_FN.sub(class_total_price, discount_price);
  console.log(class_total_price, discount_price, res);
...
function fillDiscount($obj_tr, $obj_dir, dis_price) {
  console.log($obj_tr.data('params-discount'));
  ...
  console.log(discount_price, discount_amount);
...
function xiaojiMoney($dir) {
  ...
  var p_discount = $dir.find('.js-dy-outstanding');
  var sum = 0;
...
function FMTrHTML($list, callback) {
  ...
  $.each($list, function (key, item) {
    var t_ = $(this);
    var data = t_.data('sdata');
    var discount_after = Number(t_.find('.js-dy-outstanding').val());
...
function sumTotalPayment() {
  var sum = 0;
  // 所报课程的合计
  SignClassHTML.find('.js-dy-outstanding').each(function (key, item) {
    var t_ = $(this);
...
function submitStageData(baseInfo, payInfo) {
  ...
  $('#signClassHTML .direction-item').each(function (key, item) {
    ...
    c_stage.each(function (key2, item2) {
      ...
      // pay
      var inp_discount = t_.find('.js-dy-outstanding').val() * 1;
        ...
        if (res.code == '0') {
          ...
          if (checkNeedPayIndex) {
            ...
            if ('ZFFS19' == pay_method) {
              ...
              cumParentWinModal(
                '支付信息',
                CONTEXT_PATH + '/newsignup/payIndexVer?versionCode=' + $('#versionCode').val(),
                {
                  other: {
                    closeBtn: 0
                  }
...
$('#loanIndex').on('ifChecked', function () {
  ...
  function discountsIfSubsidy() {
    ...
    return $.get(CONTEXT_PATH + '/discount/checkDiscountWithLoan?ids=' + temp.join(','));
  }
});
