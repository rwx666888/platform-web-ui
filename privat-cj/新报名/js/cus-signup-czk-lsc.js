    ...
    for (var i = 0; i < res.length; i++) {
      arr.push(
        '<option value="' +
          res[i].schoolName +
          '" ' +
          (res[i].schoolName == schoolValue ? 'selected' : '') +
          '>' +
          res[i].schoolName +
          '</option>'
      )
...
function lianbaoHTML() {
  return (
    ...
    '<button type="button" class="btn btn-sm btn-primary js-btn-lb" data-toggle="tooltip" data-placement="bottom">连报优惠</button>' +
    '</div>' +
    '<div class="form-group form-group-sm">' +
    '<label>最大减免：</label>' +
...
function submitStageData(baseInfo, payInfo) {
  ...
  $('#signClassHTML .direction-item').each(function(key, item) {
    ...
    c_stage.each(function(key2, item2) {
      ...
      tmp_obj.teachingMaterialCost = Number(t_dir.find('.js-dy-gooks').val())
      //综报订单号
...
// !200319新增
// 构建 证书 HTML
function createCertificateHTML(list) {
  ...
  list.forEach(function(row) {
    ...
    if (!tbNode.length) {
      SignCertificateHTML.html(outWrpHTML)
      SignCertificateHTML.find('.js-table-wrp').html(tableHTML)
...
// 构建 其他缴费 HTML
function createOtherPayHTML(list) {
  var trHTML = function(row) {
    var htztName = $('#payLegalSubjectName').val()
  ...
  list.forEach(function(row) {
    ...
    if (!tbNode.length) {
      SignOtherPayHTML.html(outWrpHTML)
      SignOtherPayHTML.find('.js-table-wrp').html(tableHTML)
...
// 构建 教具 html
function createTeachingHTML(list) {
  var trHTML = function(row) {
    return $(
      '<tr class="js-tr-item">' +
        '<td>' +
  ...
  list.forEach(function(row) {
    ...
    if (!tbNode.length) {
      SignTeachingHTML.html(outWrpHTML)
      SignTeachingHTML.find('.js-table-wrp').html(tableHTML)
...
function tsClassOrCertificateDiscount(totalAmount, disRatio, balanceDis) {
  // 比较可用的优惠金额
  var compareValue = function(balanceDis, val) {
    var tmp = null
    if (balanceDis > val) {
      tmp = val
      balanceDis = COM_FN.sub(balanceDis, val)
    } else {
      tmp = balanceDis
      balanceDis = 0
    }
...
function tsClassOrCertificateDiscount_bj(totalAmount, disRatio, balanceDis) {
  function compareValue(balanceDis, val) {
    var tmp = null
    if (balanceDis > val) {
      tmp = val
      balanceDis = COM_FN.sub(balanceDis, val)
...
function checkZFFSTips(type) {
  ...
  if (type == 'ZFFS07' || type === 'ZFFS14') {
    ...
    if (SignCertificateHTML.children().length) {
      COM_TOOLS.alert('证书缴费不允许使用百度贷支付')
      return false
    }
  }
  return true
...
$('#eventMain')
  .on('click', '#btnCenter', function() {
    ...
    cumParentWinModal(
      ...
      CONTEXT_PATH + '/center/toChooseCenter',
      {
        ...
        callback: {
          fn1: function(params) {
            getBtnListByOrg($('#orgCore_code').val())
          }
        }
      }
    )
  ...
  .on('click', '.js-btn-lb', function() {
    ...
    if (res_lb.res) {
      ...
      cumParentWinModal('连报优惠', CONTEXT_PATH + '/newsignup/signupNewMul?' + $.param(params), {
        callid: {
...
function discountInterest() {
  ...
  $.ajax({
    type: 'post',
    url: CONTEXT_PATH + '/discount/interest/validate',
    async: false,
    data: {
      orgCoreCode: $('#orgCore_code').val(),
...
function getBtnListByOrg(code) {
  $.post('/newsignup/findPayMethodListByCenter', {
    orgCoreCode: code
  }).then(function(res) {
    $('#eventMain')
      .find('.js-pay2-method')
