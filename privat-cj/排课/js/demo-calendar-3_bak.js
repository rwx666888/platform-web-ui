...
var cus_FN_cj = {
    ...
    comModDayTime: function(sday, type, epoi){
        if(type == 1){  //这是删除
            ...
            var tarDay_ = this.special_check_day(); //获取对应类型下的最后时间
            var tarArr = cus_TOOLS_data.get_search_data('eStartday', tarDay_);  //获取目标天所有数据
            ...
            sday >= tarDay_ ? copyArr_ = [] : sday = tarDay_;    //判断删除的是否是最后一天，自动后移
            this.autoAddDayTime(sday, epoi, copyArr_);
        }else if(type == 2){ //这是增加
            cus_TOOLS_data.del_dataSource_fun('', epoi);
        }
        this.sortNum();
    },
    autoAddDayTime: function(sda_, poi, yArr){
        ...
        while(poi>0){
            ...
            for(var i = 0; i < tlen_; i++){
                ...
                calender_plu_tool.add_ca_event(sda_, CLASS_obj_['teaId'], CLASS_obj_['teaName'], 1, CLASS_obj_.cType, '', [xArr_[i]]);
            }
            yArr=[];
    ...
    checkTeachAjax: function(ajaxParam, callback){
        $.getJSON('/arrangeCourse/queryLecturerClash', ajaxParam, function(res){
            if(res.index == 0){ //不冲突
                callback && callback();
            }else if(res.index == 1){
                if(res.list.length){
                    ...
                    for(var i = 0, ilen = list_.length; i < ilen; i++){
                        html_arr.push('&nbsp;',list_[i]['cName'],'&nbsp;',list_[i]['dateClash'],'&nbsp;',list_[i]['eTimeClash'],'&nbsp;发生冲突<br>');
                    }
                    COM_TOOLS.alert('<p class="cus-alert-right">该讲师在'+ html_arr.join('') +'</p>');
                }
            }
        });
    }
...
function creat_ca_event_fun() { /*创建整天课程*/
    ...
    calender_plu_tool.add_ca_event(sda_, tea_, teaN_, daynum_, 2, desc_, child_arr); //强制短期班
    cus_FN_cj.sortNum(); //课次排序
    calender_plu_tool.close_cal_win();
    refetch_ca_draw();
    
    /*cus_FN_cj.checkTeachAjax({
        teaId: tea_,
        timeBeginStr: sda_,
