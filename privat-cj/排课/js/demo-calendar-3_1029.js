...
var cus_FN_cj = {
    init_classData_fn: function (obj) {
        ...
        $('#add_class_time').click(function () {
            ...
            tar_.find('.js-sel-day-time:last').val(CLASS_obj_.child[0]['eTime']).trigger('change');
        });
    ...
    toObjArr: function (obj, sda_) {
        ...
        $.each($(obj).find('.calendar-class-time'), function (i, n) {
            ...
            o['eId'] = moment(sda_ + 'T' + o['eStartTime']).valueOf() + '-' + COM_TOOLS.get_random_fun(6); //模拟事件
            arr.push(o);
        });
        return arr;
    },
    ...
    autoAddDayTime: function (sda_, poi, yArr, param) {
        ...
        while (poi > 0) {
            var tlen_ = yArr.length || CLASS_obj_.child.length,
                xArr_ = yArr.length ? yArr : $.extend(true, [], CLASS_obj_.child);
    ...
    check_time_interval: function (s_data, param) {
        var flag = { res: false };
        this.arrayObjectSortBy2(s_data, 'eStartTime', 1);
        var curDay = moment().format('YYYY-MM-DD');
        $.each(s_data, function (key, val) {
            var cur_ = val;
            var cur_next = s_data[key + 1];
            if (cur_next && moment(curDay + 'T' + cur_.eEndTime).isAfter(curDay + 'T' + cur_next.eStartTime)) {
    ...
    check_date_interval: function (s_data, cdate, daynum) {
        var flag = { res: false };
        for (var i = 0, len = daynum; i < len; i++) {
            var t_date = moment(cdate).add(i, 'd').format('YYYY-MM-DD');
            var t_date_arr = this.get_interval_data(t_date, t_date);
            t_date_arr = $.grep(t_date_arr, function (n, i) {
                return n.eId && (n.eId !== s_data[0].old_eId)
            })
...
function creat_ca_event_fun(evt_) { /*创建整天课程*/
    ...
    var child_arr = cus_FN_cj.toObjArr('#calendar_win_child_box', sda_);
    var conflict_day_check = cus_FN_cj.check_time_interval(child_arr);
    if (conflict_day_check.res) {
        COM_TOOLS.alert('所选上课时段冲突！');
        return false;
    }
    var conflict_date_check = cus_FN_cj.check_date_interval(child_arr, sda_, daynum_);
    ...
    var est_ = $('.js-t-start:visible');
    var ent_ = $('.js-t-end:visible');
    for (var i = 0, ilen = eT_.length; i < ilen; i++) {
        ajaxD_.push({
            ...
            eTime: eT_.eq(i).val(),
            eStartTime : est_.eq(i).val(),
            eEndTime : ent_.eq(i).val(),
            classDateStr: sda_
        });
    }
...
function edit_ca_event(evt_) { //修改单个事件
    ...
    var n_eStartTime = $('#jsevent_win_edit').find('.js-t-start').val();
    var n_eEndTime = $('#jsevent_win_edit').find('.js-t-end').val();
    var n_classDateStr = $('#jsevent_win_date_val').val();
    ...
    var child_arr = cus_FN_cj.toObjArr('#event_dat_time', sday_);
    child_arr[0].old_eId = old_eId_;
    // var new_ePoi = cus_FN_cj.sumTotaltime(child_arr, true);
    // var old_ePoi = cus_FN_cj.sumTotaltime([{ 'eStartTime': old_eStartTime_, 'eEndTime': old_eEndTime_ }], true);
    var conflict_date_check = cus_FN_cj.check_date_interval(child_arr, sday_, 1);
    // ((old_eStartday_ != sday_) || (old_eStartTime_ != n_eStartTime) || (old_eEndTime_ != n_eEndTime))
    if (((old_eStartday_ != sday_) || (old_eStartTime_ != n_eStartTime) || (old_eEndTime_ != n_eEndTime)) && conflict_date_check.res) {
        COM_TOOLS.alert('与&nbsp;' + conflict_date_check.date + '&nbsp;上课时段冲突！');
        return false;
    }
    //TODO
    var ajaxD_ = [{
        ...
        timeEndStr: sday_,
        eTime: eTime_,
        eStartTime: n_eStartTime,
        eEndTime: n_eEndTime,
        classDateStr: n_classDateStr
    }];
    checkTeachAjax(evt_, { queryInfo: JSON.stringify(ajaxD_) }, function () {
        var tArr = $.extend(true, [], cus_TOOLS_data.get_search_data('eId', id_))[0];  //深拷贝，引用类型
        ...
        if (nEpoi > tArr['ePoi']) { //增加当前天时间
            cus_FN_cj.comModDayTime(tArr['eStartday'], 2, (nEpoi - tArr['ePoi']));
        } else if (nEpoi < tArr['ePoi']) { //减少当前天时间
            cus_FN_cj.comModDayTime(tArr['eStartday'], 1, (tArr['ePoi'] - nEpoi));
        } else {
            cus_FN_cj.sortNum();
        }
            ...
            if (e_tar_.hasClass('cus-label-xs')) { //删除按钮
                ...
                var ind_ = cumCurWinModal('课程', win_, '', {
                    ...
                    success: function (layer) {
                        $(layer).find('.js-sel-day-time').val(CLASS_obj_.child[0]['eTime']).trigger('change')
                    }
                });
                win_.data('win_ind', ind_);
