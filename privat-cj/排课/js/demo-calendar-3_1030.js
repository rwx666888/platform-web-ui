...
var cus_TOOLS_data = {
  ...
  del_dataSource_fun: function (arr, num_, param) {
    ...
    if (a_ >= 0) {
      ...
      lastEvent['eEndTime'] = cus_FN_cj.sumEndTime(lastEvent['eStartTime'], -a_, lastEvent['eClassTime'], lastEvent['eRest']);
      lastEvent['ePoi'] = -a_;
      lastEvent['estate'] = 2;
    }
  }
...
var calender_plu_tool = {
  ...
  add_ca_event: function (sda_, tea_, teaN_, daynum_, cType_, desc_, classChild_, param) {
    ...
    if (!calender_plu_tool.check_cur_day(sda_)) { // 判断时候有 sda_
      return false;
    }
    ...
    for (var i = 0, len = parseInt(daynum_); i < len; i++) {
      ...
      if (cType_ == CL_DEFAULE_CONFIG.cType.d1) { // 常规班
        sday_ = moment(sday_).add(1, 'weeks').format('YYYY-MM-DD'); //速度较慢 1ms
      } else {
        sday_ = moment(sday_).add(1, 'days').format('YYYY-MM-DD'); //速度较慢 1ms
      }
...
var cus_FN_cj = {
  ...
  initWinModalData: function (obj, data) {
    ...
    $t_.find('#calendar_win_tea_val').val(data.eTeaN || CLASS_obj_.teaName);
    $t_.find('#calendar_win_tea_val_hide').val(data.eTea || CLASS_obj_.teaId);
    $t_.find('.js-class-minute').val(data.eClassTime || CLASS_obj_.child[0].eClassTime);
    ...
    if ((data.ePoi || CLASS_obj_.child[0].ePoi) > 1 && parseInt(tsRest)) {
      $t_.find('.js-isrest[value=1]').prop('checked', 'checked').removeAttr('disabled');
      $t_.find('.js-total-rest').removeAttr('disabled');
    } else {
  ...
  sumEndTime: function (sTime, singleTimes, classHour, restHour) {
    ...
    return moment('2019-06-27 ' + sTime).add(val_, 'm').format('HH:mm');
  },
  // 重新初始化开始结束时间的选择
  ...
  getSourceData: function (obj, val_) { //根据类型获取原数据里对应的类型
  ...
  comModDayTime: function (sday, type, epoi) {
    if (type == 1) {  //减少时间，要在数组中补时间
      ...
      if (!(sday >= lastDay_)) { //判断删除的是否是最后一天，自动后移
        sday = lastDay_;
        //只判断最后一天是否有空位
    ...
    } else if (type == 2) { //增加时间，所以要从原数组中减少
  ...
  autoAddDayTime: function (sda_, poi, yArr, param) {
    ...
    while (poi > 0) {
      ...
      for (var i = 0; i < tlen_; i++) {
        var item = xArr_[i]
        if (poi > 0 && poi < item['ePoi']) {  // 不足以创建完整的一条时
          // todo 这要按照 SCH 修改
          ...
          item['eEndTime'] = cus_FN_cj.sumEndTime(item['eStartTime'], poi, item['eClassTime'], item['eRest']);
        }

        if (poi <= 0) break;
        poi = poi - item['ePoi'];

        calender_plu_tool.add_ca_event(sda_, CLASS_obj_['teaId'], CLASS_obj_['teaName'], 1, CLASS_obj_.cType, '', [item], param);
      }
      yArr = [];
  ...
  special_check_day: function (cType_) {  //按类型返回正确的末尾日期
    var lArr = {},
  ...
  check_time_interval: function (check_date, param) {
    ...
    $.each(check_date, function (key, val) {
      ...
      if (cur_next && moment(cur_.eStartday + 'T' + cur_.eEndTime).isAfter(cur_.eStartday + 'T' + cur_next.eStartTime)) {
  ...
  // 构造检验讲师冲突的数据
  checkTeacherData: function (s_list, teaId, sDate, dayNum) {
    var arr = [];
    ...
    for (var i = 0, len = s_list.length; i < len; i++) {
      arr.push({
        ...
        teaId: teaId,
        timeBeginStr: sDate,
        timeEndStr: moment(sDate).add((dayNum - 1), 'days').format('YYYY-MM-DD'),
...
function creat_ca_event_fun(evt_) {
  ...
  if (!tea_ || !teaN_ || !daynum_ || daynum_ <= 0 || parseInt(daynum_, 10) != daynum_) {
    COM_TOOLS.alert('请完整填写数据，天数为整数');
    return false;
  }
  ...
  var ckeck_qt = cus_FN_cj.filterArr(child_arr, function(item){
    return item.eStartday == moment().format('YYYY-MM-DD') && item['eStartTime'] < moment().add(cus_FN_cj.tqDate, 'h').format('HH:mm')
  })
  if(ckeck_qt.length){
  ...
  // * 检测对应时间讲师冲突
  var ajaxD_ = cus_FN_cj.checkTeacherData(child_arr, tea_, sda_, daynum_);
  checkTeachAjax(evt_, { queryInfo: JSON.stringify(ajaxD_) }, function () {
    cus_TOOLS_data.del_dataSource_fun('', curAddTotal); //* 删除对应课时数

    calender_plu_tool.add_ca_event(sda_, tea_, teaN_, daynum_, 2, desc_, child_arr, true); // 添加事件，强制短期班
...
function edit_ca_event(evt_) {
  ...
  if (dataModfiy) { // 只有修改了才触发改变
    // 允许修改当前天的特殊处理
    if(nEvent.eStartday == moment().format('YYYY-MM-DD') && nEvent.eStartTime < moment().add(cus_FN_cj.tqDate, 'h').format('HH:mm')){
      COM_TOOLS.alert('距上课时间小于' + cus_FN_cj.tqDate + '小时，禁止修改');
      return false;
    }
    ...
    //TODO
    var ajaxD_ = cus_FN_cj.checkTeacherData(child_arr, tea_, sday_, 1);
    checkTeachAjax(evt_, { queryInfo: JSON.stringify(ajaxD_) }, function () {
      cus_TOOLS_data.set_data_byid(id_, { //这修改会影响 tArr
        eId: moment(sday_ + 'T' + nEvent['eStartTime']).valueOf() + '-' + COM_TOOLS.get_random_fun(6),
        eStartday: sday_, //日期
        eTea: tea_, //讲师ID
        eTeaN: teaN_, //讲师名字
...
function calendar_init_fun() {
  var calendar_ = $('#calendar').fullCalendar({
    ...
    dayClick: function (date, jsEvent, view) {
      ...
      if (e_tar_.hasClass('cus-label-xs')) { // *删除全天按钮
    ...
    eventClick: function (cEvent, jsEvent, view) {
      ...
      if (cEvent['eStartday'] < moment().format('YYYY-MM-DD')) {
        ...
        cus_TOOLS_data.cur_sdate_ = '';
        COM_TOOLS.alert('修改日期不能小于当前日期！');
      } else if (cEvent['eStartday'] == moment().format('YYYY-MM-DD') && cEvent.eStartTime < moment().add(cus_FN_cj.tqDate, 'h').format('HH:mm')) {
...
$(document).on('change.sclasshour', '.js-single-classhour', function () {
  // 单次课时
  var t_ = $(this);
  var val = t_.val();

  if (val == 1) {
    cus_FN_cj.winTar.find('.js-isrest[value=0]').prop('checked', 'checked').trigger('change.isrest');
    setTimeout(function () {
