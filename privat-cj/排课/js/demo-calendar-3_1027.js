...
var cus_FN_cj = {
    ...
    bindSelect: function(obj){
        ...
        tar_.off().on('change', '.js-sel-day-time', function(e){
            ...
            if(t_.val() != '-1'){
                var html_ = cus_FN_cj.initSelectOpt(t_.val());
                p_.find('.js-t-start').html(html_.join('')).trigger('change');
            }else{
                t_.val(t_.data('last'));
        ...
        }).on('change', '.js-t-start', function(){  //时间
            var t_ = $(this);
            var p_ = t_.closest('.calendar-class-time');
            var sdata_ = cus_FN_cj.getSourceData(p_.find('.js-sel-day-time').val());
            
            
            /*var sTime = $(this).val().split(':');
            var hour_ = Number(sTime[0]) + Number(sdata_.ePoi);
            var scend_ = sTime[1];
            var html_ = '<option value="'+ (hour_<10 ? '0'+ hour_ : hour_) +':'+ scend_ +'">'+ (hour_<10 ? '0'+ hour_ : hour_) +':'+ scend_ +'</option>';
            p_.find('.js-t-end').html(html_);*/
            
            var cDay = moment().format('YYYY-MM-DD');
            var sTime = t_.val();
            var eTime = moment(cDay +'T'+ sTime).add(sdata_.ePoi, 'h').format('HH:mm');
            var html_ = '<option value="'+ eTime +'">'+ eTime +'</option>';
            p_.find('.js-t-end').html(html_);
        });
    },
    initSelectOpt: function(dt){
        var st, et, arr = [];
        var sdata_ = this.getSourceData('', dt);
        dt == 1 ? (st = 8, et = (12 - sdata_.ePoi)) : dt == 2 ? (st = 12, et = (18 - sdata_.ePoi)) : (st = 18, et = (22 - sdata_.ePoi));
        for(var i = st; i <= et; i++){
            arr.push('<option value='+(i<10 ? '0'+i : i)+':00>'+(i<10 ? '0'+i : i)+':00</option>');
            if(i + 1 <= et) arr.push('<option value='+(i < 10 ? '0'+i : i)+':30>'+(i < 10 ? '0'+i : i)+':30</option>');
        }
        if((et - parseInt(et)) == 0.5){
            var i_et = parseInt(et);
            arr.push('<option value='+(i_et < 10 ? '0'+i_et : i_et)+':30>'+(i_et < 10 ? '0'+i_et : i_et)+':30</option>');
        }
        return arr;
    },
...
function edit_ca_event_winf(cEvent) { //修改单个事件弹窗
    ...
    var ind_ = cumCurWinModal('课程', win_, '', {
        ...
        success: function() {
            ...
            .end().find('.js-t-start').val(cEvent['eStartTime']).trigger('change');
        },
        end: function() {
