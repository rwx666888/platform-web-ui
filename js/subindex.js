...
function cumGetParentWinGlobel(partOBJ) {
  if (window.cum_Modalindex_ > 0) {
    return cumCheckPwin(partOBJ).window["layui-layer-iframe" + window.cum_Modalindex_];
...
function cumGetParentBodyGlobel(partOBJ) {
  if (window.cum_Modalindex_ > 0) {
    return cumCheckPwin(partOBJ).layer.getChildFrame('body', window.cum_Modalindex_);
  } else if (cumCheckPwin(partOBJ) == top && top.$('.js-iframe-comwin').length > 0) {
...
var CUS_PLUGINS = {
  ...
  /**
   * JQUERY 插件，自定义面板panel
   * @param {Object} element JQ选择器
   * @param {Object} options 面板 配置项, 支持data-*模式；
   * @example
   ...
   *   </div>
   *   <div class="cus-panel-body">Panel body</div>
   * </div>
   *
   */
  _CUSPANEL_: function($) {
        ...
        $.each(btn, function(action, n) {
          if (n && n.iconClass) {
            ...
            if (/^(fullscreen|reload|collapse|close)$/.test(action)) { //内部api
              $btnbox.on('click.com.' + that.type + '.data-api', '.js-evt-btnbar[data-active="' + action +
                '"]', $.proxy(that[action], that));
            } else { //自定义按钮
              $btnbox.on('click.com.' + that.type + '.data-api', '.js-evt-btnbar[data-active="' + action +
                '"]', $.proxy(function(e) {
    ...
    Cuspanel.prototype.fullscreen = function(action) {
      ...
      var $curBtn = action instanceof $.Event ? $(action.currentTarget) : this.$btnbox.children(
        '.js-evt-btnbar[data-active="fullscreen"]');
      this.$element.toggleClass('fullscreen');
      btnClass && $curBtn.children('i').toggleClass(this.ckeckIcon(btnClass[0])).toggleClass(this.ckeckIcon(
        btnClass[1]));
    ...
    Cuspanel.prototype.reload = function(action) {
      ...
      var $curBtn = action instanceof $.Event ? $(action.currentTarget) : this.$btnbox.children(
        '.js-evt-btnbar[data-active="reload"]');
    ...
    Cuspanel.prototype.collapse = function(action) {
      ...
      var btnClass = thisBtnOpt.iconClass && thisBtnOpt.iconClass.length > 1 ? thisBtnOpt.iconClass : false;
      var $curBtn = action instanceof $.Event ? $(action.currentTarget) : this.$btnbox.children(
        '.js-evt-btnbar[data-active="collapse"]');
    ...
    Cuspanel.prototype.close = function(action) {
      ...
      this.$element.remove();
      var $curBtn = action instanceof $.Event ? $(action.currentTarget) : this.$btnbox.children(
        '.js-evt-btnbar[data-active="close"]');
   ...
   *
   */
  _CUSDATERANGEPICKER_: function($) { //TODO
...
var COM_TOOLS = {
  version: '6.3.1', //年index|月index|本月内版本index
  /* 当前版本默认语言（！！ 禁止修改 ！！） */
  _language: 'zh-CN',
  /* ### option start ### */
  /**
  ...
  initjSDefaultOpt: function(jsArr) {
    ...
    (jsArr == 'ALL' || $.inArray('jstree', jsArr) !== -1) && $.jstree && (COM_TOOLS.arrayUniquePushFn(COM_TOOLS[
      'cache_obj']['_jsfileArr'], 'jstree'), $.jstree.root = COM_DEFAULT._jsTreeOpt['root']);
    /*修改validate中文*/
    (jsArr == 'ALL' || $.inArray('validator', jsArr) !== -1) && $.validator && (CUS_PLUGINS['_VALID_'].init(), $.extend(
        $.validator.messages, TEDU_LANGUAGE['_validate']()[COM_TOOLS['_language']]),
      COM_TOOLS.arrayUniquePushFn(COM_TOOLS['cache_obj']['_jsfileArr'], 'validator'));
    /*修改datetimepicker默认配置*/
    (jsArr == 'ALL' || $.inArray('datetimepicker', jsArr) !== -1) && $.fn.datetimepicker && (COM_TOOLS.arrayUniquePushFn(
        COM_TOOLS['cache_obj']['_jsfileArr'], 'datetimepicker'),
    ...
    (jsArr == 'ALL' || $.inArray('daterangepicker', jsArr) !== -1) && $.fn.daterangepicker && ($.fn.daterangepicker
      .defaultOptions = {}) && (COM_TOOLS.arrayUniquePushFn(COM_TOOLS['cache_obj']['_jsfileArr'],
        'daterangepicker'),
      $.extend($.fn.daterangepicker.defaultOptions, COM_DEFAULT._dateRangePickerOpt, {
        locale: TEDU_LANGUAGE['_daterangepicker']()
      }));
    /*修改select2中文*/
    (jsArr == 'ALL' || $.inArray('select2', jsArr) !== -1) && $.fn.select2 && (TEDU_LANGUAGE['_select2'](), $.fn.select2
      .defaults.set('language', COM_TOOLS['_language']),
      $.fn.select2.defaults.set('width', 'style'), COM_TOOLS.arrayUniquePushFn(COM_TOOLS['cache_obj'][
        '_jsfileArr'
      ], 'select2'));
  },
  /* ###  option end  ### */
  ...
  is_playerDay: function(sday_) {
    ...
    return ((cday_ == 0 || cday_ == 6 || $.inArray(sday_, COM_TOOLS._data.PLAYDAYS_) != -1) && $.inArray(sday_,
      COM_TOOLS._data.WEEKDAYS_) == -1)
  },
  /**
   * 检验时间  ,获取下一个工作日
   * @param sday_待检测的时间
   ...
   *  API:getSelectRowsData fn(name) 获取选中行的数据，name:列名，【可选】，不指定则返回整行的数据；否则只返回指定列的数据; return 一个数组对象[];  
   */
    ...
    var _table = $('#' + domid).DataTable($.extend(true, {
      ...
      initComplete: function(settings, json) {
        ...
        if (settings._select && settings._select.style == 'mutil') { //全选、反选
          $(settings.nTableWrapper).off('click.dtcus', '.cus-checkbox-all').on('click.dtcus',
            '.cus-checkbox-all',
            function() {
          ...
          _that.off('select.dt.dtcus deselect.dt.dtcus').on('select.dt.dtcus deselect.dt.dtcus', function(e,
            dt, type, indexes) { //全选、反选联动
            if (dt) {
        ...
        if ($.isFunction(opt['jsTrDblclick'])) { //暂不支持对冻结列的双击操作
          ...
          $(settings.nTBody).off('dblclick.dtcus touchstart.dtcus touchmove.dtcus touchend.dtcus', 'tr').on({
            ...
            'touchstart.dtcus': function(e) {
              if (!e.originalEvent || !e.originalEvent.changedTouches || !e.originalEvent.changedTouches[
                  0]) {
                return;
              }
            ...
            'touchmove.dtcus': function(e) {
              if (cto && e.originalEvent && e.originalEvent.changedTouches && e.originalEvent.changedTouches[
                  0] &&
                (Math.abs(ex - e.originalEvent.changedTouches[0].clientX) > 50 || Math.abs(ey - e.originalEvent
                  .changedTouches[0].clientY) > 50)) {
                clearTimeout(cto);
              }
            },
          ...
          }, 'tr').off('dblclick.dtcus', '.select-checkbox, :input, .no-Sel-obj').on('dblclick.dtcus',
            '.select-checkbox, :input, .no-Sel-obj',
            function() {
              return false;
            });
        }
   ...
   * @param {Object} dt (new datatable)实例对象
   * @param {String} name 选填，需要获取的字段名称，为空返回整个data对象（非绑定的数据）；
    ...
    dt.columns(selector).nodes().to$().each(function(i, n) {
      $(this).filter(function(s, n) {
        ...
        if (!$(this).children().is('input.js-getval')) {
          var txt = $(this).text();
          var inp = $("<input name='" + selector[i].split(':')[0] +
            "' class='input-sm js-getval no-Sel-obj' type='text'>");
          inp.val(txt);
          $(this).html(inp);
        }
      });
  ...
  select2_init: function(id, url, ajaxtype, datafn, opt) {
    ...
    opt = $.extend(true, {}, $.type(url) == "object" ? url : ($.type(url) == "array" && $.type(ajaxtype) ==
      "object" ? ajaxtype : ($.type(opt) == "object" ? opt : {})));
    ...
    function thatMatcher_(term, text) {
      if (text.toPinYin != undefined) {
        return text.toPinYin().indexOf(term.toUpperCase()) >= 0 ? true : text.toUpperCase().indexOf(term.toUpperCase()) >=
          0;
      }
   ...
   */
  jstree_init: function(selector, opt) {
    ...
    if (_dopt.input_val_obj.length) { //如果有 $input_val 对象，则直接使用
      ...
      $input_val = $.type(selector) == "string" ? $('#' + selector + '_val') : $jstree.nextAll(
        'input.jstree_init_val:hidden');
    }
    if (_dopt.input_text_obj.length) { //如果有 $input_text 对象，则直接使用
      $input_text = _dopt.input_text_obj;
    } else {
      $input_text = $.type(selector) == "string" ? $('#' + selector + '_text') : $jstree.nextAll(
        'input.jstree_init_text:hidden');
    }
          ...
          'core': {
            'data': {
              ...
              'success': function(res) {
                !!_dopt['groupName'] && res && res.length && (COM_TOOLS.cache_obj._cus_jstree_cache[_dopt[
                  'groupName']] = res);
              }
            }
          }
  ...
  confirm: function(title, opt, yes, cancel) {
    ...
    return l_ && (cancel = yes, yes = opt),
      layer.confirm(title || '', $.extend({
        zIndex: layer.zIndex,
        btn: [LOCAL_MESSAGE_DATA["platform.plugin.com_btn.confirm"], LOCAL_MESSAGE_DATA[
          "platform.plugin.com_btn.cancel"]],
    ...
    _obj_.loadFile = function(u, callback) {
      ...
      if (COM_TOOLS['_jsFileList'][u]['css']) {
        if (!_obj_.checkCssIsLoad(COM_TOOLS['_jsFileList'][u]['css'])) {
          ...
          thatLink_.before($('<link rel="stylesheet" type="text/css" href="' + COM_TOOLS['_jsPath'].replace(
            '/js/', '/css/') + COM_TOOLS['_jsFileList'][u]['css'] + '"/>'));
        }
      }
    };
    _obj_.checkCssIsLoad = function(p_) {
      return !!$('head link[rel="stylesheet"]').filter(function() {
        return p_.substring(p_.indexOf('css/')) == $(this).attr('href').substring($(this).attr('href').indexOf(
          'css/'));
      }).length;
    };
    ...
    if (jQuery.type(id) === "string" && $('#' + id).length) {
      ...
      if (isiframe_) {
        ...
        $(
          ...
          '.flex-tabpage .tab-content{height: 100%;padding-top: 42px;}.flex-tabpage .tab-content .tab-pane{height:100%;}</style>'
        ).appendTo("head");
      }
          ...
          if (cbox_.length > 0) {
            if (isiframe_) {
              ...
              if ($iframe_.length) {
                ...
                COM_TOOLS.loadingOverlay.open(cbox_);
                $iframe_ = $(
                  '<iframe scrolling="auto" allowtransparency="true" style="height:100%;width:100%;display:block;" id="tabmodl_' +
                  cont_ + '" name="tabmodl_' + cont_ +
  ...
  infoTipsFn: function(selector, target, callback, opt) {
    var _defaults = {
      ...
      content: function() { //注意title为空时，此方法会执行两次；
        ...
        if (/<img[^>]*/.test(html_)) {
          _defaults['height'] = _defaults['maxHeight'] ? _defaults['maxHeight'] : $(window).height() * (
            /left|right/.test(_defaults['placement']) ? .5 : .4) + 'px';
        }
        return html_;
      }
    };
    $.extend(_defaults, (opt || {}));
    _defaults['template'] = '<div class="popover cus-popover-box" role="tooltip" style="max-width:' + (_defaults[
        'maxWidth']) + ';' +
      (_defaults['imageZoom'] ? 'width:' + (_defaults['maxWidth']) + ';' : '') +
      '"><div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-content" style="' +
        ...
        var _showTimer = window.setTimeout(function() {
          _this.on('inserted.bs.popover', function() {
            ...
            if (_defaults['imageZoom']) {
              $('#' + _this.attr('aria-describedby')).find('img').wrap(
                '<span class="img-zoom-box"></span>').css('display', 'block').parent().zoom();
            }
          });
          var _thisPop = _this.addClass('js-mouseove').popover(_defaults).popover('show').attr(
            'aria-describedby');
  ...
  cus_mult_choice: function(id, btnfn, num, id_1, id_2) {
    ...
    var obj_ = {
      addItem: function(id, text) {
        ...
        t_.append('<span data-text="' + text + '" data-code="' + id + '" class="mult-item">' + text +
          ' <i class="glyphicon glyphicon-remove js-mult-remove"></i></span>');
  ...
  _view: { //视图工具
     ...
     */
    setInputStatus: function(obj, status, noIcon) {
      if (obj) {
    ...
    cusPopover: function(id, opt, startcb, endcb) {
      var param_ = {
        ...
        width: '',
        html: true,
        title: '<button type="btn" class="btn btn-warning btn-sm cus-pop-close" onclick="javascript:$(\'#' + id +
          '\').trigger(\'click\')"><i class="glyphicon glyphicon-chevron-up"></i></button>'
      };
      var tar_ = '',
        rep_ = '';
      if ($.type(opt) == "object") {
     ...
     */
    cusFileUpload: function(id, hide_id, option) {
      var $obj = $('#' + id);
      if (!$obj.length) {
        return false;
      }
      ...
      var option_ = $.extend({
        ...
        isname: false, // 是否显示文件名
        uptype: '',
        filetype: [], // 格式类型
        filesize: ((btn_img == 'btn' ? COM_DEFAULT._fileServeOpt.defaultFileSize : COM_DEFAULT._fileServeOpt.imgFileSize) ||
          5 * 1024), // 文件大小,单位：KB
      }, option);
      ...
      if (!option_.filetype.length) {
        option_.filetype = btn_img == 'btn' ?
          COM_DEFAULT._fileServeOpt.fileType.slice() :
          btn_img == 'img' ? COM_DEFAULT._fileServeOpt.imgType.slice() : [];
      }
      ...
      if ($obj.hasClass('js-dy-creation')) {
        ...
        var $rep_html = $('<div id="' + ('js_is' + btn_img + '_' + t_id) + '" class="cus-single-upload">' +
          ...
          '<input id="' + inp_h_id + '" name="' + inp_h_id + '" ' + (option_.required == true ? 'required' : '') +
          ' class="hide-inp-validate" value="' + f_path + '" type="text"/>' +
          '<input class="js-inp-files hidden" type="file" accept="' + ($.map(option_.filetype, function(dif_) {
            return '.' + dif_;
          })) + '"/>' +
          obj_html +
          '</div>' +
          (option_.isname ? '<span class="file-name"></span>' : '') +
          '</div>');
        ...
        // 回显时 btn 显示名称
        if (btn_img == 'btn' && f_path && option_.isname) {
          var f_name = '';
     ...
     */
    cus_dropdown: function(selector, opt) {
      opt = opt || {};
      opt = $.extend(true, { //配置项
      ...
      _obj.show = function() {
        ...
        if (opt.autoPlace) {
          ...
          placement = placement == 'bottom' && pos.bottom + actualHeight > viewportDim.bottom ? placement.replace(
              'bottom', 'top') :
     ...
     *
     */
    cus_drop_jstree: function(id, hide_id, opt) {
      opt = opt || {};
      opt = $.extend(true, { //配置项
      ...
      var _obj = {};
      var $wrapBox = opt.wrapBoxId ? $.type(opt.wrapBoxId) == 'string' ? $('#' + opt.wrapBoxId) : (opt.wrapBoxId.selector ?
        opt.wrapBoxId : $textObj) : $textObj;
      ...
      var CUS_DROPDOWN_ = COM_TOOLS._view.cus_dropdown($wrapBox, $.extend(true, {}, opt.dropdown, { //内容不允许被修改
        content: (opt.hasSearch ? '<input class="jstree-searchbox form-control input-sm"/>' : '') +
          '<div class="js-dy-jstree"></div>',
      }));
    ...
    noviceGuide: '<div class="popover cus-step-popover-box" role="tooltip"> <div class="arrow"></div> <h3 class="popover-title"></h3> <div class="popover-content"></div>' +
      '<div class="popover-navigation"> <div class="btn-group"> <button class="btn btn-sm btn-default" data-role="prev">&laquo; ' +
      LOCAL_MESSAGE_DATA["platform.plugin.msg.prevStep"] +
      '</button> <button class="btn btn-sm btn-default" data-role="next">' + LOCAL_MESSAGE_DATA[
        "platform.plugin.msg.nextStep"] + ' &raquo;</button>' +
      '<button class="btn btn-sm btn-default" data-role="pause-resume" data-pause-text="Pause" data-resume-text="Resume">' +
      LOCAL_MESSAGE_DATA["platform.plugin.msg.pause"] +
      '</button> </div> <button class="btn btn-sm btn-default" data-role="end">' + LOCAL_MESSAGE_DATA[
        "platform.plugin.msg.end"] + '</button> </div> </div>',
    /* 标签滚动 */
    tabScrollFn: {
      dom_: {
        ul_: $(),
    ...
    cusPWD: function(selector, isDynamicDom) {
      ...
      if (isDynamicDom != false) {
        ...
        $obj.before(temp);
        var $rep_html = $(
          '<div class="input-group input-group-sm js-pwd-parent"> \
                <span class="js-replace-dom"></span> \
  ...
  _model: {
    set_i18n_fn: function(cboxid, geturl, seturl) {
      ...
      var tabBoxHtml_ = '<div class="js-cus-tabmodel-box" id="js_cus_tabmodel_box">' +
        ...
        '<button class="btn btn-primary btn-sm js-cus-btn-save js-helpmsg" type="button" data-helpmsg="platform.plugin.com_msg.tab_this" data-helpdata="' +
        TEDU_MESSAGE.get("platform.plugin.com_btn.save") + '">' +
        '<i class="glyphicon glyphicon-floppy-disk"></i> ' + TEDU_MESSAGE.get('platform.plugin.com_btn.save') +
        '</button>' +
        '<button class="btn btn-warning btn-sm js-cus-btn-refresh" type="button"><i class="glyphicon glyphicon-refresh"></i> ' +
        TEDU_MESSAGE.get('platform.plugin.com_btn.fresh') + '</button>' +
        '</div>' +
        '</div>' +
      ...
      function _createTabHtml(index, data) {
        ...
        var tab_ = '<li role="presentation" class="' + (index == 0 ? 'active' : '') + '"><a href="#js_tab_' + str_ +
          '" role="tab" data-toggle="tab">' + data['name'] + '</a></li>';
        var cont_ = '<div role="tabpanel" class="tab-pane ' + (index == 0 ? 'active' : '') + '" id="js_tab_' + str_ +
          '" data-pid="' + data['pid'] + '">' + _Backfill(data['child']) + '</div>';
        tabBox_.append(tab_);
      ...
      function _Backfill(data) {
        if ($.type(data) == 'array' && data.length > 0) {
          ...
          $.each(data, function(j, m) {
            ...
            arr.push('<label class="col-sm-2 col-md-offset-2 control-label">', (m.required == '1' ?
              '<span class="text-danger">*</span> ' : ''), m.name, '</label>');
            arr.push(
              '<div class="col-sm-5"><input class="form-control input-sm js-input-val" type="text" value="',
              m.value, '" ', (m.required == '1' ? 'required="required"' : ''), '/></div>');
            arr.push('</div>');
          });
          arr.push('</div>');
          return arr.join('');
     ...
     */
    set_i18n_fn2: function(cboxid, formid, formbtn) {
        ...
        COM_TOOLS.ajaxFn({
          ...
          success: function(d) {
            if (d && $.type(d) == 'array') {
              ...
              contBOx_.find('input.js-input-val[required]').off('blur.i18n.check').on('blur.i18n.check',
                function() {
                  _checkInput($(this));
                });
            }
      ...
      function _createTabHtml(data) {
        ...
        var tab_ = '<li role="presentation" class="' + (index_ == 0 ? 'active' : '') + '"><a href="#js_tab_' + str_ +
          '" role="tab" data-toggle="tab">' + data['name'] + '</a></li>';
        var cont_ = '<div role="tabpanel" class="tab-pane ' + (index_ == 0 ? 'active' : '') + '" id="js_tab_' +
          str_ + '" data-pid="' + data['pid'] + '">' + _Backfill(data['child'], data['oldpid']) + '</div>';
        tabBox_.append(tab_);
        contBOx_.append(cont_);
      }
      ...
      function _addEventListener(data) { //资源名称与默认翻译语言联动
        ...
        form_.find('[js-i18n-key]').filter(function() {
          var that_ = $(this);
          return _toLowerCase_key(that_.attr('js-i18n-key')) == oldpid_ && (_thisSelect ? that_.is(_thisSelect) :
            true);
        ...
        if ($.type(data) == 'array' && data.length > 0) {
          ...
          $.each(data, function(j, m) {
            ...
            arr.push(
              '<div class="col-sm-5"><input class="form-control input-sm js-input-val" type="text" value="',
              m.value, '" ',
              (m.required == '1' ? 'required="required"' : ''), 'js-i18n-target="', _toLowerCase_key(oldpid),
              '_', m.code, '" data-i18n_isedit="', (!!m.value), '" /></div>');
            arr.push('</div>');
          });
          arr.push('</form>');
          return arr.join('');
      ...
      function _saveFn(shadeType, shadeOpt, callback) { //保存
        ...
        if (!look_) {
          COM_TOOLS.ajaxFn({
            ...
            success: function(d) {
              if (d.code == '1') { //成功
                cumCheckPwin(parent).COM_TOOLS.alert(TEDU_MESSAGE.get('platform.plugin.com_msg.i18n') +
                  TEDU_MESSAGE.get('platform.plugin.com_msg.success'));
                $.type(callback) == 'function' ? callback() : cumParentCallValue(); //如果不设置回调函数，则默认关闭窗口
              } else if (d.code == '-2') { //必须含有简体中文
                COM_TOOLS.alert(TEDU_MESSAGE.get('platform.plugin.com_msg.i18n') + TEDU_MESSAGE.get(
                  'platform.plugin.com_msg.china_notnull'));
              } else { //操作失败
                COM_TOOLS.alert(TEDU_MESSAGE.get('platform.plugin.com_msg.i18n') + TEDU_MESSAGE.get(
                  'platform.plugin.com_msg.defeated'));
              }
              _checkSaveBtn(true);
            },
            error: function() {
    ...
    msg_fn2: function(data, opt) {
      ...
      function make_c(other_ids) { //构造消息
        ...
        $.each(data, function(i, n) {
          ...
          content_.push('<i class="glyphicon ', (n.type == '1' ? 'glyphicon-bell' : 'glyphicon-bullhorn'),
            '"></i>');
          content_.push('</div>');
          content_.push('<div class="cus-msg-rbox">');
          content_.push('<h3>', n.title, '</h3>');
          if (COM_DEFAULT._isOpenNotice.version == '1.0') {
            content_.push('<div class="msg-cont201103">', n.msg, '</div>');
          }
    ...
    panel: {
      ...
      add_one_panel: function(m, $target, action) {
        var modelHtml = [];
        modelHtml.push('<div class="cus-panel ', (m.skin && m.skin.className ? m.skin.className :
          'cus-panel-default'), '" data-itemid="', m.id, '" data-iframesrc="', m.url, '">');
        ...
        modelHtml.push(
          '<div class="cus-panel-body"><p class="panel-no-permission"><i class="glyphicon glyphicon-exclamation-sign"></i><span>您还没有[',
          m.name, ']模块的访问权限！</span></p></div></div>');
        if (action == 'makeAndCache' || action == 'makeOne') {
          (action == 'makeAndCache' ? $target.find('#' + m.parent) : $target).children('.cus-col-box').append(
            COM_TOOLS._model.panel.model_wrap(modelHtml.join(''), m.id, m.modelid, m.parent, 3).data('skin', m.skin)
          );
          if (action == 'makeOne') {
      ...
      model_wrap: function($html, itemid, model_id, parentid, itemtype) {
        var $wrap = $('<div class="js-model-item" data-wrapitem_id="' + itemid + '"  data-model_id="' + model_id +
          '" data-parent_id="' + parentid + '" data-model_type="' + itemtype +
          '"><div class="js-code-box"></div>' +
     ...
     * @param {Object} opt 配置属性;
     * {
     *   type: 1：组织与人员（默认）；2：组织；
     *   only_sel_people: （默认:false）是否只可选择人员;仅"type==1"时有效;
     *   cb: 点击确定按钮的回调函数,返回当前所有选定的数据,可直接入库存储,不包含扩展数据属性;
     *   max_num: 最大选择数目, -1为无限制（默认）；
	 *   including_resigned: 是否包含已离职人员，【可选】，-1：不包含已离职的人员（默认），1：包含已离职的人员；2：包含已离职的人员和已禁用的部门；
     * }
     */
    orgModel: function(editbtn_selector, data_selector, opt) {
    ...
    _orgModel_: function(opt) {
      ...
      obj._openModel = function(selected_list, yesCallBack) {
        ...
        var JSTREE;
        /* 组件容器 */
        var $wrapper = $('<div style="display: none;" class="cus-orgjtree-wrap" id="' + UID_ +
          '"><div class="select-treelist-xxw">' +
          '<div class="clearfix">' +
        ...
        function makeSelectedItem(itemdata) {
          ...
          return $('<div title="' + itemdata.path + '" data-id_="' + itemdata.id + '" class="item-xxw1121 ' +
            (itemdata.hasauth == '0' ? 'nohasauth' : (itemdata.dtype == '0' && itemdata.isexternal == '2' ?
              'this-outer-org' : '')) + ' clearfix">' +
                ...
                if (res) {
                  ...
                  if (res.person && res.person.length > 0) {
                    ...
                    $.each(res.person, function(i, n) {
                      str += '<p class="search-item clearfix js-search-item" title="' + n.path +
                        '" data-code="' + n.code + '" data-path="' + n.path + '" data-isexternal="' +
                        n.isexternal +
                  ...
                  if (res.department && res.department.length > 0) {
                    ...
                    $.each(res.department, function(i, n) {
                      str += '<p class="search-item clearfix js-search-item ' + (n.isexternal == '2' ?
                          'this-outer-org' : '') + '" title="' + n.path +
                        '" data-code="' + n.code + '" data-path="' + n.path + '" data-isexternal="' +
                        n.isexternal +
                        '" data-dtype="' + n.type + '" data-text="' + n.text + '" data-id="' + n.id +
                        '">' +
                        '<i class="iconx  glyphicon glyphicon-folder-close"></i><span>' + n.text +
                        '</span>' +
                        '</p>';
                    });
                    str += '</div></div>';
      ...
      obj._updateSelItem = function(dlist, $sel_wrap) {
        ...
        $.each(dlist, function(i, n) {
          str += '<li class="orgtree-item ' + (n.type == '0' && n.isexternal == '2' ? 'this-outer-org' : '') +
            '" data-code="' + n.code + '" data-type="' + n.type + '"' +
            (n.path ? 'title="' + n.path + '"' : '') + '>' +
            '<i class="the-icon glyphicon ' + (n.type == '1' ? 'glyphicon-user' : 'glyphicon-folder-close') +
            '"></i>' + n.text +
            '</li>';
        });
...
var COM_DEFAULT = {
  ...
  "_isOpenNotice": {
    ...
    "version": '1.0',
    "showMsg": 0, //控制顶部消息轮播是否显示
    "showWait": 0, //控制顶部待办事项是否显示
    "showNotice": 0, //控制系统通知是否显示,
    "showRbnotice": 0, //控制右下角弹框是否启用
    "appKey": '',
    "getNewurl": '', //接口地址
    "processListUrl": '', //待办消息地址
    "rongcoudflag": "0" //是否启用融云， 0 或 1；
  },
  /* 自定义模块——国际化模块 */
  "_modelI18nOpt": {
    "geturl": "", //获取数据接口地址
