...
var COM_TOOLS = {
    version: '5.5.1', //年index|月index|本月内版本index
    /* 当前版本默认语言（！！ 禁止修改 ！！） */
    _language: 'zh-CN',
    /* ### option start ### */
    /**
    ...
    _model: {
            ...
            function _saveFn(shadeType, shadeOpt, callback) { //保存
                ...
                if (!look_) {
                    COM_TOOLS.ajaxFn({
                        ...
                        success: function (d) {
                            if (d.code == '1') { //成功
                                parent.COM_TOOLS.alert(TEDU_MESSAGE.get('platform.plugin.com_msg.i18n') + TEDU_MESSAGE.get('platform.plugin.com_msg.success'));
        ...
        orgModel: function (editbtn_selector, data_selector, opt) {
            ...
            opt = $.type(opt) == "object" ? opt : {};
            /* 选择结果的放置容器，如果未定义容器则不处理 */
            var $sel_dataInput = $.type(data_selector) == 'string' ? $('#' + data_selector) : (data_selector.selector ? data_selector : $());
            /* 编辑按钮 */
            var $edit_btn = $.type(editbtn_selector) == 'string' ? $('#' + editbtn_selector) : (editbtn_selector.selector ? editbtn_selector : $());
            if ($edit_btn.length == 0 || $sel_dataInput.length == 0 || $edit_btn.data('orgmodel')) {
                return false;
            }
            var selected_list = JSON.parse($.trim($sel_dataInput.val()) || "[]");
            var _option = {
            ...
            function _openModel() {
                selected_list = JSON.parse($.trim($sel_dataInput.val()) || "[]"); //必须
                var JSTREE;
                /* 组件容器 */
                ...
                function _init_jsfn() {
                    $tree.jstree({
                        ...
                        if(node.node.children.length > 0) {
                            $.each(node.node.children, function(i, n) {
                                $selBox.find('.item-xxw1121').each(function(a, b) {
                                    var id = $(this).data('id_');
                                    if(n == id) {
                                        JSTREE.select_node(id, true, false)
                                    }
                                });
                            })
                        }*/
                ...
                $btn_yes.click(function () {
                    ...
                    if (_option.max_num > 0 && arr.length > _option.max_num) {
                        parent.COM_TOOLS.alert('最多选择' + _option.max_num + '个');
                        return false;
                    }
                    _initSelBox(sArr);
                    // 全量覆盖，保障一致性，简单粗暴
            ...
            /* 默认选择结果框容器 */
            var $sel_wrap = $('<div class="orgtree-selectedlist-box">' +
                '<ul class="orgtree-item-list clearfix"></ul>' +
                '</div>');
            $sel_dataInput.before($sel_wrap).addClass('hidden');

            function _makeSelbox(dlist) {
                var str = '';
             ...
             * 构造选择结果框
             * @param {Object} data
             * @param {Boolean} outside 是否外部数据
             */
            function _initSelBox(data_, outside) {
                // console.log('#-1', $sel_dataInput, data_);
            ...
            $edit_btn.click(_openModel);
            _initSelBox(selected_list, true);
