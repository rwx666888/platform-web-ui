...
defaultOptions = {
	...
	chart: {
		...
		//spacingLeft: 10,
		style: {
			fontFamily: '"Lucida Grande", "Lucida Sans Unicode", Verdana, Arial, Helvetica, sans-serif', // default font-crm
			fontSize: '12px'
		},
	...
	tooltip: {
		...
		headerFormat: '<span style="font-crm-size: 10px">{point.key}</span><br/>',
...
SVGRenderer.prototype = {
	...
	buildText: function (wrapper) {
		var textNode = wrapper.element,
			...
			lines = pick(wrapper.textStr, '').toString()
				.replace(/<(b|strong)>/g, '<span style="font-crm-weight:bold">')
				.replace(/<(i|em)>/g, '<span style="font-crm-style:italic">')
				.replace(/<a/g, '<span')
				...
				if (span !== '' || spans.length === 1) {
					...
					if (span !== ' ') {
						...
						if (!spanNo && lineNo) {
							...
							// Set the line height based on the font-crm size of either
	...
	/**
	 * Utility to return the baseline offset and total line height from the font-crm size
	 */
	fontMetrics: function (fontSize) {
		fontSize = pInt(fontSize || 11);

		// Empirical values found by comparing font-crm size and bounding box height.
		// Applies to the default font-crm family. http://jsfiddle.net/highcharts/7xvn7/
...
Axis.prototype = {
	...
	defaultOptions: {
		...
		title: {
			...
			style: {
				color: '#4d759e',
				//font-crm: defaultFont.replace('normal', 'bold')
				fontWeight: 'bold'
			}
			//x: 0,
			//y: 0
...
defaultPlotOptions.scatter = merge(defaultSeriesOptions, {
	lineWidth: 0,
	tooltip: {
		headerFormat: '<span style="font-crm-size: 10px; color:{series.color}">{series.name}</span><br/>',
