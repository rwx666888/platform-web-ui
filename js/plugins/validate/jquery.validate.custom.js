 ...
 * 添加特殊规则 "ck-number-len", "ck-number-forjqnum"的参数处理 @lianglei #2017-12-22
 * 扩展resetform 方法，添加表单重置能力 form.reset() @lianglei #2019-06-12
 */
(function( factory ) {
...
$.extend( $.validator, {
	...
	prototype: {
		...
		resetForm: function() {
			if ( $.fn.resetForm ) {
			    ...
			    this.currentForm.reset(); // @梁磊 添加重置表单功能；
			}
			this.invalid = {};
			this.submitted = {};
			this.prepareForm();
	...
	normalizeRules: function( rules, element ) {
		...
		$.each( [ "ck-number-len", "ck-number-forjqnum"], function() {// 添加规则  @梁磊 2017-12-21
    		if ( rules[ this ] ) {
    		    var arr_ = rules[ this ];
                ...
                var sepL_ =  (arr_[1] && (arr_[1] && 0 <= arr_[1] && arr_[1] <= 12)) ? arr_[1] : 12,
                    depL_ = (arr_[0] && (arr_[0] && arr_[0] > 0)) ? arr_[0] : 0;
                rules[ this ] = [depL_, sepL_];
            }
        } );
        
        $.each( rules, function( k, v ) { // 添加规则  @梁磊 2017-12-22 将以规则中"ck-cus-"开头的，且其属性值为数组形字符串的，转化成数组对象
            if(k.indexOf('ck-cus-') === 0){
                if ( $.type(v) === "string" ) {
                    var str = $.trim(v);
                    if(str && str.substring(0, 1) == "[") {
                        rules[ k ] = (new Function("return " + str))();
                    }
                }
            }
        } );
...
///<jscompress sourcefile="zdy-validate.js" />
/*!
 * 自定义验证规则扩展
