...
var cus_FN_cj = {
    ...
    initSelectOpt: function (obj, dt) {
        var s, e, arr = [];
        // dt == 1 ? (s = 6, e = 12) : dt == 2 ? (s = 12, e = 18) : (s = 18, e = 24);
        (s = 6, e = 24)
        for (var i = s; i < e; i++) {
    ...
    comModDayTime: function (sday, type, epoi) {
        if (type == 1) {  //这是删除
            ...
            var lastArr = cus_TOOLS_data.get_search_data('eStartday', lastDay_);  //获取目标天所有数据
            var copyArr_ = $.extend(true, [], CLASS_obj_.child); //深拷贝
            //只判断最后一天是否有空位
            for (var i = 0, ilen = lastArr.length; i < ilen; i++) {
                for (var j = 0, jlen = copyArr_.length; j < jlen; j++) {
                    if (copyArr_[j]['eTime'] == lastArr[i]['eTime']) {
                        copyArr_.splice(j, 1);
                        break;
                    }
                }
